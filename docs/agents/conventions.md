...
- **Assertion library is FluentAssertions — keep it.** All unit tests assert with FluentAssertions (plus xUnit as the framework and Verify for snapshots). FluentAssertions v8 prints a build warning that it is under the Xceed commercial license "for commercial use only" — **this does not apply to us.** Fallout is an open-source project and uses FluentAssertions under that allowance (its maintainer, Dennis Doomen, grants open-source use). Do **not** swap it out for Shouldly/AwesomeAssertions or downgrade to dodge the warning, and don't re-raise the licensing warning as an issue.
- **`Fallout.Core.Tests` is an xunit.v3 + ArchUnitNET pilot.** It is the one project on **xunit.v3** (everything else is xunit v2). It deliberately uses the **`xunit.v3.mtp-off`** package so it runs under **VSTest**, not Microsoft.Testing.Platform — because the rest of the repo is VSTest and the .NET 10 SDK treats a mixed VSTest+MTP solution as an unsupported scenario, and C# Dev Kit only discovers MTP projects behind an experimental flag. It also opts out of central package management (`ManagePackageVersionsCentrally=false`) and self-declares its versions; opting out of CPM is the switch that also excludes it from the shared xunit-v2 deps block in `Directory.Build.props`. **Architecture-fitness tests use ArchUnitNET** (`TngTech.ArchUnitNET` + `.xUnitV3`), not NetArchTest.
- **Future: full Microsoft.Testing.Platform is a repo-wide move, not per-project.** Switching to "modern MTP" means plain `xunit.v3` (drop `mtp-off`) **and** a `global.json` `"test": { "runner": "Microsoft.Testing.Platform" }` entry — which flips *every* test project at once. Only do it when all test projects move to xunit.v3 together; don't half-migrate.
