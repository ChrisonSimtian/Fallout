...
Tool wrappers (the `.json` schemas most likely to be extended) live under `src/Tools/Fallout.Application.Tools.<Family>/<Tool>/<Tool>.json`, grouped into per-family assemblies (`Fallout.Application.Tools.<Family>`, namespace `Fallout.Application.Tools.<Family>.<Tool>`).
