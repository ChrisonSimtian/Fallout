...
// This test lives under Fallout.Application.Tools.*, so the bare name GitVersion would otherwise bind to
// the sibling namespace (Fallout.Application.Tools.GitVersion) rather than the record of the same name.
// The alias must sit *inside* the namespace scope to outrank the enclosing sub-namespace during lookup.
using GitVersion = Fallout.Application.Tools.GitVersion.GitVersion;
