...
// This test lives under Fallout.Infrastructure.CI.*, so each bare provider name (AppVeyor, …) would
// otherwise bind to the sibling namespace (Fallout.Infrastructure.CI.AppVeyor) rather than the host
// type of the same name. These aliases must sit *inside* the namespace scope so they outrank the
// enclosing CI sub-namespaces during simple-name lookup — above the namespace line they'd lose to them.
using AppVeyor = Fallout.Infrastructure.CI.AppVeyor.AppVeyor;
using Bitrise = Fallout.Infrastructure.CI.Bitrise.Bitrise;
using TeamCity = Fallout.Infrastructure.CI.TeamCity.TeamCity;
using AzurePipelines = Fallout.Infrastructure.CI.AzurePipelines.AzurePipelines;
using Jenkins = Fallout.Infrastructure.CI.Jenkins.Jenkins;
using TravisCI = Fallout.Infrastructure.CI.TravisCI.TravisCI;
using GitLab = Fallout.Infrastructure.CI.GitLab.GitLab;
