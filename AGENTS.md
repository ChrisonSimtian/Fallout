...
**Rebrand status:** the structural rename has landed — namespaces (`Fallout.*`), package IDs, project filenames, and the global tool name (`dotnet fallout`) are all in place. The old `Nuke.*` transition shims have been **removed**: the onion re-layering ([ADR-0006](docs/adr/0006-onion-layering-and-namespace-realignment.md)) is too deep to shim honestly, so it is a deliberate one-time break — NUKE consumers migrate via the `fallout-migrate` CLI (`src/Fallout.Migrate`; a fuller migration helper is in progress). See [docs/rebrand-plan.md](docs/rebrand-plan.md) for the namespace mapping.
...
- **[docs/agents/repository-layout.md](docs/agents/repository-layout.md)** — full directory structure, project groupings, migration strategy
