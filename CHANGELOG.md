...
- **TFS leftover stripped** from `nuget.config` and `src/Nuke.GlobalTool/templates/nuget.config` (`<solution><add key="disableSourceControlIntegration"/></solution>` — only relevant under TFVC, useless on git).
