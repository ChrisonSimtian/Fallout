...
- **⚠️ Tool wrappers moved to per-family namespaces: `Fallout.Common.Tools.<Tool>` → `Fallout.Application.Tools.<Family>.<Tool>`** (onion Application-layer alignment). The ~65 tool wrappers were split into 14 per-family assemblies (`Fallout.Application.Tools.<Family>`) in an earlier PR; this aligns their **namespaces** to the assembly so they're governed by the architecture-fitness naming rule. E.g. `Fallout.Common.Tools.Docker` → `Fallout.Application.Tools.Containers.Docker`, `…Tools.Slack` → `…Application.Tools.Notifications.Slack`; the `DotNet` tool collapses to `Fallout.Application.Tools.DotNet` (family == tool).
  - **Migration / impact**: update `using` / `using static` directives — e.g. `using static Fallout.Common.Tools.DotNet.DotNetTasks;` → `using static Fallout.Application.Tools.DotNet.DotNetTasks;`. Full per-tool → family mapping in [docs/rebrand-plan.md](docs/rebrand-plan.md). The shared base tooling namespace (`Fallout.Common.Tooling`: `ToolTasks`, options/attributes) is **unchanged**.
  - **Pre-release break**: lands on the `2026` line before its first release ([ADR-0007](docs/adr/0007-cut-release-branch-on-demand.md)), so no stable consumer is stranded. Compatibility shims for the old `Fallout.Common.Tools.*` namespaces are deferred (follow-up).
  - **Migrate-tooling follow-up**: the `Nuke`→`Fallout` rewriter (`Fallout.Migrate[.Analyzers]`) still rewrites tool types to `Fallout.Common.Tools.*`; updating it to the family namespaces is tracked with the shim work.
...
- **Architecture-fitness suite — `tests/Fallout.Architecture.Specs`** (#95). A solution-wide [ArchUnitNET](https://github.com/TNG/ArchUnitNET) suite that locks in the runtime layering so future work can't silently invert a dependency edge that still compiles. Rules are scoped by **assembly** (the legacy NUKE namespaces span assemblies, so namespace-based layering would be meaningless): foundation purity (`Fallout.Core` / `Fallout.Utilities` depend on nothing else in-repo), utility-satellite confinement, downward-only layering for `Tooling`/`ProjectModel`/`Build.Shared`/`Solution`, "nothing depends on the `Fallout.Cli` composition root", and "`Fallout.*` never depends on the `Nuke.*` shims". Known-broken areas are governed by a **one-way ratchet**: each rule is allowed exactly its documented baseline of pre-existing violations (`KnownViolations`), and the test fails on any *new* violation or any baseline entry that has been fixed — so the debt can only shrink. The headline debt rule (a type's namespace should be rooted at its assembly name) ships with a ~220-entry baseline capturing today's `Fallout.Common.*` namespace sprawl, which the onion-architecture refactor will whittle down. Migrated the issue-#88 `Fallout.Core` purity guard off the unmaintained `NetArchTest.Rules` (last release 2023) onto ArchUnitNET and retired that package. See [docs/architecture-tests.md](docs/architecture-tests.md).
