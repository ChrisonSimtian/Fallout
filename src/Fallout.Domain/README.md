...
`Fallout.Domain` is the innermost ring of Fallout. It holds the immutable domain shape of the build execution pipeline and the pure graph algorithms that schedule it:
...
- No reference to any other Fallout project (not even the `Fallout.Core` shared kernel).
- No dependency on `System.IO`, `System.Diagnostics.Process`, `Console`, or Serilog.
- No statics holding mutable state.

If you need to touch the filesystem, spawn a process, write to the console, or log — that logic belongs in `Fallout.Application` or higher, not here. The orchestration that *calls* these algorithms (reading parameters, failing the build on a cycle, mutating target state) lives in `Fallout.Application`; the domain only computes.
