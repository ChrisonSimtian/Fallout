...
public sealed class TransitionShimGenerator : IIncrementalGenerator
{
    ...
    private static string EmitTopLevelShim(INamedTypeSymbol type, ShimMarker marker)
    {
        ...
        // If the canonical type (or any member we delegate to) is marked
        // [Experimental], the generated references would trip the opt-in
        // diagnostic. The shim mirrors the canonical faithfully, so it suppresses
        // those exact IDs around the whole type — a Nuke.* consumer reaching an
        // experimental surface through the shim inherits the same opt-in the
        // canonical demands.
        var experimentalIds = CollectReferencedExperimentalIds(type);
        ...
        if (experimentalIds.Count > 0)
            sb.Append("#pragma warning disable ").AppendLine(string.Join(", ", experimentalIds));
        EmitTypeBody(sb, type, indentLevel: 1);
        if (experimentalIds.Count > 0)
            sb.Append("#pragma warning restore ").AppendLine(string.Join(", ", experimentalIds));
        sb.AppendLine("}");
        return sb.ToString();
    }

    /// <summary>
    /// Collects the <c>[Experimental]</c> diagnostic IDs that the generated shim
    /// would reference: the canonical type itself (base clause + ctor base-calls)
    /// and any public static method we delegate to. Returned sorted for stable output.
    /// </summary>
    private static SortedSet<string> CollectReferencedExperimentalIds(INamedTypeSymbol type)
    {
        var ids = new SortedSet<string>(StringComparer.Ordinal);
        CollectExperimentalIds(type, ids);
        foreach (var member in type.GetMembers())
        {
            if (member is IMethodSymbol { IsStatic: true, MethodKind: MethodKind.Ordinary } method
                && method.DeclaredAccessibility == Accessibility.Public)
                CollectExperimentalIds(method, ids);
        }
        return ids;
    }

    private static void CollectExperimentalIds(ISymbol symbol, SortedSet<string> ids)
    {
        foreach (var attr in symbol.GetAttributes())
        {
            if (attr.AttributeClass?.ToDisplayString() != "System.Diagnostics.CodeAnalysis.ExperimentalAttribute")
                continue;
